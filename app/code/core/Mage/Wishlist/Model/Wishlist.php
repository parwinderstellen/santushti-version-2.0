...
class Mage_Wishlist_Model_Wishlist extends Mage_Core_Model_Abstract
{
    ...
    protected function _addCatalogProduct(Mage_Catalog_Model_Product $product, $qty = 1, $forciblySetQty = false)
    {
        ...
        if ($item === null) {
             //$abc=$product->getCustomOptions();
             $abc=  serialize($_SESSION['product_sku']['Custom Size']);
             ...
             if(Mage::getSingleton('customer/session')->isLoggedIn()) 
            {
                                   ...
                                   $sql1        = "select count(*) as cnt from wishlist_custom_size where customer_id=$customer_id and product_id=".$product->getId();
                               ...
                               {         
                                          ...
                                          $sql        = "insert into wishlist_custom_size values('',".$product->getId().",".$this->getId().",$customer_id,'".$abc."')";
   ...
   //  $items[]=$_SESSION[product_sku]['Custom Size'];
