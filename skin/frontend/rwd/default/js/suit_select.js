...
//alert($path);
 jQuery('#save_data').click(function(){
		...
		var inSeamLength = jQuery('#inSeamLength').val();
                var armHole = jQuery('#armHole').val();
                var neckDeepFront = jQuery('#neckDeepFront').val();
                var neckDeepBack = jQuery('#neckDeepBack').val();
                var thigh = jQuery('#thigh').val();
                var knee = jQuery('#knee').val();
                var mori = jQuery('#mori').val();
   ...
   if(jQuery("#measurementForm").valid())
   {
			...
			jQuery.post($path+'measurement',{
					...
					inSeamLength: inSeamLength,
                                        armHole:armHole,
                                        neckDeepFront:neckDeepFront,
                                        neckDeepBack:neckDeepBack,
                                        thigh:thigh,
                                        knee:knee,
                                        mori:mori,
         ...
         jQuery('.save_data').click(function(){
             $sku_id=jQuery(this).attr("id");
				...
				var inSeamLength = jQuery('#'+$sku_id+'_inSeamLength').val();
                                ...
                                var armHole = jQuery('#'+$sku_id+'_armHole').val();
                                var neckDeepFront = jQuery('#'+$sku_id+'_neckDeepFront').val();
                                var neckDeepBack = jQuery('#'+$sku_id+'_neckDeepBack').val();
                                var thigh = jQuery('#'+$sku_id+'_thigh').val();
                                var knee = jQuery('#'+$sku_id+'_knee').val();
                                var mori = jQuery('#'+$sku_id+'_mori').val();
   ...
   if(jQuery("#measurementForm_"+$sku_id).valid())
   {
		...
		jQuery(".custom_size_modal_"+$sku_id).hide();	
              ...
              var tool_html="<a class='tooltip_cart'  title='<div id=drees_measures><h2>Dress Measures</h2> <ul><li><span>Neck Deep Front - "+neckDeepFront+" cm</span><span>Neck Deep Back - "+neckDeepBack+" cm</span></li><li><span>Sleeve Length - "+sleeveLength+" cm</span><span>Across Shoulder - "+acrossshoulder+" cm</span></li><li><span>Arm Hole - "+armHole+"cm</span><span>Bust - "+bust+" cm</span></li><li><span>Waist - "+waist+"  cm</span><span>Hip - "+hip+" cm</span></li><li><span>Length - "+length+" cm</span><span>To Fit Waist - "+toFitWaist+" cm</span></li><li><span>To Fit Hip - "+toFitHip+" cm</span><span>Out Seam Length -  "+outSeamLength+" cm</span></li><li><span>In Seam Length - "+inSeamLength+"cm</span><span>Thigh - "+thigh+" cm</span></li><li><span>Knee- "+knee+"cm</span><span>Mori - "+mori+" cm</span></li></ul>' href='#'>View Meaurement</a></div>";        
